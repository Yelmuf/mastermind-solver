...
int inputCP(int def) {
  /// Handle input of number from 1 to 9
  ...
  while(1) {
    ...
    if(n > 0 && n < 10) {
      ...
      printf("[!] Your input may have caused overflow. Using default number (%i)\n", def);
      break;
    }
  }
  return def;
...
_Bool handleInput(long input, int m, int p) {
    for (int i = 0; i < p; ++i) {
...
int countActive(_Bool *active, int length) {
    int count = 0;
...
int testNumber(long testingNumber, long *b, int length, _Bool *active, int p) {
    /// Find maximum score of number to test: https://programmingpraxis.com/2009/11/20/master-mind-part-2/
    int maximum = 0;
    for (int i = 0; i < 40; ++i) {
        int counter = 0;
        _Bool f = (composeNum(i, 5) / 10 + composeNum(i, 5)) < 5;
...
void setNumbersWithSameCodeActive(long number, long code, long *b, int length, _Bool *active, int p) {
...
void excludeNumbersWithSameCode(long number, long code, long *b, int length, _Bool *active, int p) {
...
int minimax(long *b, int length, _Bool *active, int p) {
    ...
    for (int i = i; i < length; ++i) {
        if (!active[i])
            ...
            if (minimum > maximum) {
                minimum = maximum;
                minimumNumber = b[i];
                printf("    new minimax<-%i ", minimumNumber);
            }
        }
    }
...
int main() {
    printf("x---------------------------------------x\n");
    printf("|     Welcome to mastermind-solver!     |\n");
    printf("x---------------------------------------x\n\n");
    ...
    _Bool active[length]; // array of _Booleans to exclude in next search
    long b[length];
    initArrayFromZero(b, length, m);

    printf("Initialized %i possibilities\n", length);
    printf("Array tail: ");
    for (int i = 5; i > 0; i--) {
...
//         << active[length-1] << " (should be all zeros)" << endl;
    ...
    while (!handleInput(input, m, p)) {
        printf("[!] Wrong code format, check number of colors and places, then enter code again:\n");
        scanf("%li", &input);
    }
    ...
    printf("Here's a starting minimax: %li\n", teste);
    ...
    for (int krok = 1; krok <= 10; krok++) {
        ...
        printf("    It's not an answer! Searching numbers to choose from ");
        if (krok == 1)
            printf("all");
        else
            printf("%i", countActive(active, length));
        printf(" active..\n    Excluding (%li, %i ...)\n", teste, r);
        ...
        if (newTeste == teste) {
            printf("\n[E] Opps, got into an infinite loop, please report to author\nExiting immediately\n");
            return 3;
        }
        teste = newTeste;
    }
